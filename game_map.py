...
class Map(object):
    ...
    def map_generation(self):
        for y_row in range(self.rows):
            for x_column in range(self.columns):
                new_tile_type = random.choice(terrain_types)
        ...
        new_buffalo = Buffalo(x_position, y_position)

        self.animals_group.add(new_buffalo)
    ...
    def generate_wheat_cluster(self):
        ...
        while not wheat_cluster_placed:
            ...
            cluster_center_wheat = Wheat(coordinates[0], coordinates[1])
            vegetation_collisions = []
            vegetation_collisions = (pygame.sprite.spritecollide(cluster_center_wheat, self.vegetation_group, False))
            if not vegetation_collisions:
                wheat_cluster_placed = True
        self.vegetation_group.add(cluster_center_wheat)
        ...
        for wheat in range(number_of_neighbors):
            ...
            while not neighbor_placed:
                ...
                new_neighbor = Wheat(coordinates[0], coordinates[1])
                vegetation_collisions = []
                vegetation_collisions = (pygame.sprite.spritecollide(new_neighbor, self.vegetation_group, False))
                if not vegetation_collisions:
                    neighbor_placed = True
            self.vegetation_group.add(new_neighbor)
